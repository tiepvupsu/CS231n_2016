...
def conv_forward_naive(x, w, b, conv_param):
  ...
  # for n in xrange(N):
  #   for f in xrange(F):
  #     w_col = w[f,:, :, :].reshape(-1, 1)
  #     for h1 in xrange(H1):
  #       for w1 in xrange(W1):
  #         x_row = x_pad[n, :, h1*stride : h1*stride + HH, w1*stride: w1*stride + WW].reshape(1, -1)
  #         out[n, f, h1, w1] = x_row.dot(w_col) + b[f]

  W = w.reshape(F, -1)
  for n in xrange(N):
    for h1 in xrange(H1):
      for w1 in xrange(W1):
        x_col = x_pad[n, :, h1*stride : h1*stride + HH, w1*stride: w1*stride + WW].reshape(-1, 1)
        out[n, :, h1, w1] = W.dot(x_col).reshape(F,) + b
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
...
def conv_backward_naive(dout, cache):
  ...
  for n in xrange(N):
    for h1 in xrange(H1):
      for w1 in xrange(W1):
        x_col = x_pad[n, :, h1*stride : h1*stride + HH, w1*stride: w1*stride + WW].reshape(-1, 1)
        tmp = dout[n, :, h1, w1].reshape(F, 1)
        dw += tmp.dot(x_col.T).reshape(w.shape)
        dx_pad[n, :, h1*stride : h1*stride + HH, w1*stride: w1*stride + WW] += \
            W.T.dot(tmp).reshape( -1, HH, WW)
...
def max_pool_forward_naive(x, pool_param):
  ...
  N, C, H, W = x.shape 
  new_h = int(np.floor((H - h)/stride)) + 1
  new_w = int(np.floor((W - w)/stride)) + 1
  out = np.zeros((N, C, new_h, new_w))

  # print x
  for n in xrange(N):
    for c in xrange(C):
      for h1 in xrange(new_h):
        for w1 in xrange(new_w):
          window = x[n, c, h1*stride:h1*stride+h, w1*stride:w1*stride+w]
          out[n, c, h1, w1] = np.amax(window)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
    ...
    for c in xrange(C):
      for h1 in xrange(new_h):
        for w1 in xrange(new_w):
          window = x[n, c, h1*stride:h1*stride+h, w1*stride:w1*stride+w]
          ii = np.argmax(window)
          ih, iw = np.unravel_index(ii, (h, w))
          dx[n, c, h1*stride + ih, w1 *stride + iw] += dout[n, c, h1, w1]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  cache = {}
  for c in xrange(C):
    tmp, cache[c] = batchnorm_forward(x[:,c,:,:].reshape(N,-1), \
      gamma[c], beta[c], bn_param)
...
def spatial_batchnorm_backward(dout, cache):
  ...
  dbeta = np.zeros(C)
  for c in xrange(C):
    A = batchnorm_backward(dout[:, c, :, :].reshape(N, -1), cache[c])
    dgamma[c] = np.sum(A[1])
    dbeta[c] = np.sum(A[2])
    dx[:, c, :, :] = A[0].reshape(N, H, W)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
